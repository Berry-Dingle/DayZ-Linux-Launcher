...
def build_sidebar_toolbar(window) -> Gtk.Widget:
    ...
    toolbar_cell.set_halign(Gtk.Align.FILL)
    toolbar_cell.set_valign(Gtk.Align.START)
    toolbar_cell.set_overflow(Gtk.Overflow.HIDDEN)

    search_header = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
    search_header.add_css_class("dzll-sidebar-toolbar-row")
    search_header.set_margin_top(11)
    ...
    window.status_refresh_progress_box = Gtk.Box(
    ...
    window.status_refresh_progress_text_row = Gtk.Box(
        orientation=Gtk.Orientation.HORIZONTAL,
        spacing=2,
    )
    window.status_refresh_progress_text_row.set_hexpand(True)
    window.status_refresh_progress_text_row.set_halign(Gtk.Align.FILL)

    window.status_refresh_progress_prefix_label = Gtk.Label(label="Refreshing:", xalign=0.0)
    window.status_refresh_progress_prefix_label.add_css_class("status-refresh-progress-label")
    window.status_refresh_progress_prefix_label.set_halign(Gtk.Align.START)
    window.status_refresh_progress_prefix_label.set_single_line_mode(True)
    window.status_refresh_progress_prefix_label.set_ellipsize(Pango.EllipsizeMode.END)
    window.status_refresh_progress_text_row.append(window.status_refresh_progress_prefix_label)

    window.status_refresh_progress_count_label = Gtk.Label(xalign=1.0)
    window.status_refresh_progress_count_label.add_css_class("status-refresh-progress-label")
    window.status_refresh_progress_count_label.set_hexpand(True)
    window.status_refresh_progress_count_label.set_halign(Gtk.Align.FILL)
    window.status_refresh_progress_count_label.set_single_line_mode(True)
    window.status_refresh_progress_count_label.set_width_chars(1)
    window.status_refresh_progress_count_label.set_max_width_chars(1)
    window.status_refresh_progress_count_label.set_ellipsize(Pango.EllipsizeMode.START)
    window.status_refresh_progress_text_row.append(window.status_refresh_progress_count_label)
    window.status_refresh_progress_box.append(window.status_refresh_progress_text_row)

    window.status_refresh_progress_bar = Gtk.ProgressBar()
    window.status_refresh_progress_bar.add_css_class("status-refresh-progress-bar")
    window.status_refresh_progress_bar.set_hexpand(True)
    window.status_refresh_progress_bar.set_halign(Gtk.Align.FILL)
    window.status_refresh_progress_bar.set_show_text(False)
    ...
    window.status_refresh_slot.set_size_request(1, -1)
    window.status_refresh_slot.set_halign(Gtk.Align.FILL)
    window.status_refresh_slot.set_valign(Gtk.Align.CENTER)
